...
if [[ "$1" = "prod" ]]; then
    ...
    docker run -dit --restart always --name ${product_container_name} -e "VERIFY_SSL=${production_ssl_verify}" -e "SLACK_BOT_TOKEN=${BOT_TOKEN}" -e "SLACK_APP_TOKEN=${APP_TOKEN}" -e "DEMISTO_URL=${DEMISTO_URL}" -e "DEMISTO_API_KEY=${DEMISTO_API_KEY}" ${product_version}
    docker logs ${product_container_name}
else
